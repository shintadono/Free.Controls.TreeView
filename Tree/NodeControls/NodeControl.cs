...
namespace Free.Controls.TreeView.Tree.NodeControls
{
	...
	public abstract class NodeControl : Component
	{
		...
		{
			return GetActualSize(node, context, -1);
		}

		internal Size GetActualSize(TreeNodeAdv node, DrawContext context, int rightBoundLastControl)
		{
			if(IsVisible(node))
			{
				Size s=MeasureSize(node, context, rightBoundLastControl);
		...
		public abstract Size MeasureSize(TreeNodeAdv node, DrawContext context, int rightBoundLastControl);
