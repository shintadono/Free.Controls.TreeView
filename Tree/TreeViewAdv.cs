...
using Free.Controls.TreeView.Threading;
...
namespace Free.Controls.TreeView.Tree
{
	...
	public partial class TreeViewAdv : Control
	{
		...
		internal IEnumerable<NodeControlInfo> GetNodeControls(TreeNodeAdv node, Rectangle rowRect)
		{
			...
			if(ShowPlusMinus)
			{
				width=_plusMinus.GetActualSize(node, _measureContext, x).Width;
			...
			if(!UseColumns)
			{
				...
				{
					Size s=c.GetActualSize(node, _measureContext, x);
					if(!s.IsEmpty)
					{
						width=s.Width;
						rect=new Rectangle(x, y, width, rowRect.Height);
		...
		private void SelectNodes(Collection<TreeNodeAdv> nodes)
		{
			...
			{
				n.IsSelected=true;
				if(n.IsExpanded) SelectNodes(n.Nodes);
			}
		}
